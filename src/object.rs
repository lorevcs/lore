...
//!
//! An [`Entry`] is one unit of intent. Its id is the hash of its `text` only, so
//! the same intent expressed twice collapses to a single object. That is what
//! makes merges deduplicate for free. A [`Commit`] groups entries and points at
//! its parents; its id covers every field, because a commit is a unique event in
//! history.
...
impl Object {
    ...
    pub fn id(&self) -> String {
        ...
        match self {
            // Identity is the intent text only, so duplicate intent deduplicates.
            Object::Entry(e) => {
                h.update(b"entry\0");
...
mod tests {
    ...
    fn entry(author: &str, ts: u64, text: &str) -> Object {
        Object::Entry(Entry {
    ...
    fn entry_id_is_intent_only() {
        // Same text but different author and time share one id.
        let a = entry("alice", 1, "use sqlite");
        let b = entry("bob", 999, "use sqlite");
        assert_eq!(a.id(), b.id());
    }

    #[test]
    fn entry_id_changes_with_text() {
        let base = entry("alice", 1, "use sqlite");
        assert_ne!(base.id(), entry("alice", 1, "use postgres").id());
    }
    ...
    fn bytes_round_trip_through_json() {
        let obj = entry("alice", 7, "make it fast");
